...
try {
    ...
    // "zawieszone" = status=40 i czas od startu >= staleMin
    // (bez heartbeat na razie, żeby było kompatybilne z obecną tabelą)
    if ($stale) {
        $where[] = "status = 40 AND pack_started_at IS NOT NULL AND TIMESTAMPDIFF(MINUTE, pack_started_at, NOW()) >= :stale";
        $bind[':stale'] = $staleMin;
    }
    ...
    $sql = "
      SELECT
        ...
        CASE
          ...
          THEN TIMESTAMPDIFF(SECOND, pack_started_at, NOW())
          ELSE NULL
        END AS age_seconds
      FROM pak_orders
      $w
