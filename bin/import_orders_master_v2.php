...
require_once __DIR__ . '/../app/Services/LegacyAuctionPhotoMap.php';
...
try {
    ...
    // Legacy image source (mysql2 / admin_automat) - opcjonalne
    $legacyImg = null;
    try {
        $mysql2 = Db::mysql2($cfg);
        $legacyImg = new LegacyAuctionPhotoMap($mysql2, $logger);
    } catch (\Throwable $e) {
        $logger("MASTER: legacy image map disabled: " . $e->getMessage());
    }

    $imp = new ImporterMasterV2($mysql, $state, $repo, $subReader, $euReader, $blReader, $cfg, $logger, $legacyImg);
    $res = $imp->run();
