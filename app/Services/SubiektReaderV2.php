...
final class SubiektReaderV2
{
    ...
    public function fetchItemsForDocs(array $docIds): array
    {
        ...
        foreach (array_chunk($docIds, 500) as $chunk) {
            ...
            $sel = [
                ...
                ($s['syncIdCol'] ? "p.{$s['syncIdCol']}" : "NULL") . " AS ob_SyncId",
    ...
    private function getSchema(): array
    {
        ...
        ($this->log)("SUBR: schema docTable={$this->schema['docTable']} posTable={$this->schema['posTable']} towTable={$this->schema['towTable']} posDocCol={$this->schema['posDocCol']} syncIdCol=" . ($this->schema['syncIdCol'] ?? 'NULL') . " twOpisCol=" . ($this->schema['twOpisCol'] ?? 'NULL') . " przelewCol=" . ($this->schema['przelewCol'] ?? 'NULL'));
        return $this->schema;
    }
    ...
    private function detectSchema(\DateTimeImmutable $from, \DateTimeImmutable $to, string $pattern, array $types): array
    {
        ...
        foreach ($docTables as $dt) {
            ...
            foreach ($posTables as $pt) {
                ...
                $syncIdCol = $this->firstCol($posCols, ['ob_SyncId']);
