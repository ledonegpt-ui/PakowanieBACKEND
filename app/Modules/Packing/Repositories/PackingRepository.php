...
final class PackingRepository
{
    ...
    /**
     * Zwraca order_code pierwszego zamowienia w batchu ktore:
     * - ma status 'picked' w picking_batch_orders
     * - NIE ma jeszcze sesji packingu ze statusem 'completed'
     * Uzywane przez endpoint GET /api/v1/packing/next
     */
    public function findNextOrderToPack(int $batchId): ?string
    {
        $st = $this->db->prepare("
            SELECT pbo.order_code
            FROM picking_batch_orders pbo
        ...
        return $row ? (string)$row['order_code'] : null;
    }

    /**
     * Sprawdza czy batch istnieje.
     */
    public function batchExists(int $batchId): bool
    {
        $st = $this->db->prepare("
            SELECT id FROM picking_batches WHERE id = :batch_id LIMIT 1
        ");
        $st->execute([':batch_id' => $batchId]);
    ...
    /**
     * Szuka nastepnego zamowienia do spakowania w batchu,
     * pomijajac biezace zamowienie (uzywane przez finishSession).
     */
