(() => {
  ...
  // Heartbeat aktywnego pakowania (żeby "zawieszone" było liczone realnie)
  ...
  const state = {
    ...
    heartbeat: { t: null, inflight: false, orderCode: null },
  };
  ...
  function stopHeartbeat(){
    if (state.heartbeat.t) clearInterval(state.heartbeat.t);
    state.heartbeat.t = null;
    state.heartbeat.inflight = false;
    state.heartbeat.orderCode = null;
  }

  async function sendHeartbeatOnce(){
    if (!isCurrentPackingMine() || !state.order) return false;
    if (state.heartbeat.inflight) return false;

    const oc = String(state.order.order_code || '').trim();
    if (!oc) return false;

    state.heartbeat.inflight = true;
    try {
      const data = await apiJson('/api/pack_heartbeat.php', {
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded'},
        body: new URLSearchParams({ order_code: oc }).toString()
      });

      if (data && data.ok) {
        // lokalnie odśwież pole z serwera (przydatne do debug/ew. UI)
        if (state.order && String(state.order.order_code || '') === oc) {
          if (data.pack_heartbeat_at) state.order.pack_heartbeat_at = data.pack_heartbeat_at;
        }
        return true;
      }
      return false;
  ...
  function syncHeartbeatLoop(){
    if (!isCurrentPackingMine() || !state.order) {
      stopHeartbeat();
      return;
    }
    ...
    // jeśli już działa dla tego samego zamówienia — nic nie rób
    if (state.heartbeat.t && state.heartbeat.orderCode === oc) return;

    stopHeartbeat();
    state.heartbeat.orderCode = oc;

    // pierwszy heartbeat od razu + potem co HEARTBEAT_MS
    sendHeartbeatOnce();
    state.heartbeat.t = setInterval(() => {
      sendHeartbeatOnce();
    }, HEARTBEAT_MS);
  }
  ...
  function setOrderView(o, items, warning){
    ...
    // Heartbeat tylko dla PACKING na swoim stanowisku
    syncHeartbeatLoop();
  }
